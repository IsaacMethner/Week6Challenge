$(document).ready(function () {
  ...
  function getCurrentWeather(loc) {
    ...
    $.ajax({
      ...
      city = `&lat=${parseInt(response1.coord.lat)}&lon=${parseInt(
        response1.coord.lon
      )}`;
      ...
      $.ajax({
        ...
        let color =
          uviTitle +
          `<span style="background-color: ${backgrdColor}; padding: 0 7px 0 7px;">${response3.value}</span>`;
        $("#currUVI").html(color);
      });
    });
  }
